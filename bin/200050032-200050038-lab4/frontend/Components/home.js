...
const HomePage = ()=>{
    ...
    async function drop_course(c_id){
        fetch('http://localhost:4000/drop/'+c_id, {
            method: 'GET',
            mode: 'cors',
            credentials: 'include',
        })
        ...
        .then(
            async (response)=>{
                if (response.drop_status === "0") navigate(0);
                else if (response.drop_status === "-1") navigate("/login");
                else if (response.drop_status === "-2") setFetchStatus(false);
            }
        )
    }
    ...
    const f=async()=>{
        ...
        .then(
            async (response)=>{
                ...
                if (response[0] === null) navigate("/instr/home")
