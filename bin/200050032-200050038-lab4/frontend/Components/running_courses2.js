...
const RunningCoursePage=()=>{
    ...
    useEffect(()=>{
    const f=()=>{
        fetch('http://localhost:4000/running_courses', {
            method: 'GET',
            mode: 'cors',
            credentials: 'include',
        })
    ...
    if (!done) return (
        ...
        <div>
            <h2 style={{textAlign: 'center'}}>Running Courses</h2>
            <table style={{borderCollapse: 'collapse', width: '80%', margin: '0 auto'}}>
                <thead>
                    ...
                    <th style={{border: '1px solid black', padding: '8px'}}>No. of Courses offered</th>
                    </tr>
                </thead>
                <tbody>
                    {Object.keys(data).map((k, i) => {
                        let n = data[k]
                        let j = "/instr/course/running/" + n.dept_name
                        return (
                            <tr style={{backgroundColor: '#f2f2f2'}}>
                                <td style={{border: '1px solid black', padding: '8px'}}><a href = {j}>{n.dept_name}</a></td>
                                <td style={{border: '1px solid black', padding: '8px'}}>{n.count}</td>
                            </tr>
                        )
                    }
                    )}
